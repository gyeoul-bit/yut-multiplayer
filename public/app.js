...
function render(s){
  ...
  if(s.pending && myTurn){
    ...
    const needsDirection=!!s.pending.directionOptions;
    $("moveText").textContent=s.pending.selectedPiece
      ? `말 ${s.pending.selectedPiece} 선택 · ${s.pending.rollName}${s.pending.roll>0?` ${s.pending.roll}칸`:''}${needsDirection&&!s.pending.choice?' · 이동 방향을 선택하세요':s.pending.choice?` · ${s.pending.choice==='diag'?'대각선':'직진'}`:''}`
      : `윷 결과: ${s.pending.rollName} · 이동할 말 1~4를 선택하세요.`;
    $("directionPanel").classList.toggle("hidden",!s.pending.selectedPiece || !needsDirection || !!s.pending.choice);
    $("straightBtn").classList.toggle("hidden",!s.pending.selectedPiece || !needsDirection || !!s.pending.choice);
    $("diagBtn").classList.toggle("hidden",!s.pending.selectedPiece || !needsDirection || !!s.pending.choice);
    $("moveBtn").classList.toggle("hidden",!s.pending.selectedPiece || (needsDirection&&!s.pending.choice) || s.pending.mergeAvailable);
    $("mergeBtn").classList.toggle("hidden",!s.pending.selectedPiece || (needsDirection&&!s.pending.choice) || !s.pending.mergeAvailable);
    $("separateBtn").classList.toggle("hidden",!s.pending.selectedPiece || (needsDirection&&!s.pending.choice) || !s.pending.mergeAvailable);
