...
const P={
  ...
  BR1:{x:76.28,y:80.27}, BR2:{x:63.50,y:61.56}, C1:{x:50,y:50.34}, TL2:{x:35.77,y:42.86}, TL3:{x:21.17,y:23.81},
  TL1:{x:21.17,y:23.81}, TL_C:{x:35.77,y:42.86}, C2:{x:50,y:50.34}, BR3:{x:63.50,y:61.56}, BR4:{x:76.28,y:80.27},
  TR1:{x:76.28,y:23.81}, TR2:{x:63.50,y:42.86}, C3:{x:50,y:50.34}, BL2:{x:35.77,y:61.56}, BL3:{x:21.17,y:80.27},
  BL1:{x:21.17,y:80.27}, BL_C:{x:35.77,y:61.56}, C4:{x:50,y:50.34}, TR3:{x:63.50,y:42.86}, TR4:{x:76.28,y:23.81}
};
...
function render(s){
  ...
  $("status").textContent=s.started
    ? (myTurn ? (s.pending ? `윷 결과: ${s.pending.rollName} · 이동할 말 ${s.pending.selectedPiece?'선택됨':'선택하세요'}${s.pending.routeAt?` · ${s.pending.routeAt.startsWith('C')?'중간점 도착':'경로 선택'}`:''}` : "내 차례입니다. 윷을 던지세요.") : `현재 차례: ${esc(s.players[s.turn]?.name||'-')}`)
  ...
  if(s.pending && myTurn){
    ...
    const needsDirection=Array.isArray(s.pending.directionOptions) && s.pending.directionOptions.length>0;
    $("moveText").textContent=s.pending.selectedPiece
      ? `말 ${s.pending.selectedPiece} 선택 · ${s.pending.rollName}${s.pending.roll>0?` ${s.pending.roll}칸`:''}${needsDirection?' · 경로를 선택하세요':''}`
      : `윷 결과: ${s.pending.rollName} · 이동할 말 1~4를 선택하세요.`;
    $("directionPanel").classList.toggle("hidden",!s.pending.selectedPiece || !needsDirection);
    $("straightBtn").classList.toggle("hidden",!s.pending.selectedPiece || !needsDirection);
    $("diagBtn").classList.toggle("hidden",!s.pending.selectedPiece || !needsDirection);
    const atCenter=String(s.pending.routeAt||"").startsWith("C");
    $("directionPanel").querySelector("span").textContent=atCenter ? "중간점에서 다음 경로를 선택하세요:" : "이동 방향을 선택하세요:";
    $("straightBtn").textContent=atCenter ? "직진" : "직진";
    $("diagBtn").textContent=atCenter ? "다른 대각선" : "대각선";
    $("moveBtn").classList.toggle("hidden",!s.pending.selectedPiece || needsDirection || s.pending.mergeAvailable);
    $("mergeBtn").classList.toggle("hidden",!s.pending.selectedPiece || needsDirection || !s.pending.mergeAvailable);
    $("separateBtn").classList.toggle("hidden",!s.pending.selectedPiece || needsDirection || !s.pending.mergeAvailable);
...
$("straightBtn").onclick=()=>socket.emit("chooseDirection",{direction:"straight"});
$("diagBtn").onclick=()=>socket.emit("chooseDirection",{direction:"diag"});
